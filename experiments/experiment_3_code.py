...
def split_by_boundaries_components(text: str, components_boundaries: dict, components_types: dict):
    """
...
def label_sentences_components(text: str, components_boundaries: dict, components_types: dict):
    """
    ...
    labeled_parts = split_by_boundaries_components(text, components_boundaries, components_types)
...
def read_brat_dataset_components(folder: str):
    """
            ...
            if file.endswith('.ann'):
                ...
                if os.path.exists(txt_path):
                    with open(ann_path, 'r', encoding='utf-8') as ann_f, open(txt_path, 'r', encoding='utf-8') as txt_f:
                        ...
                        labeled_sentences = label_sentences_components(text, components_boundaries, components_types)
...
def objective(trial, components_cnn_classifier, relations_cnn_classifier, abs_dataset_components: qp.data.Dataset, abs_dataset_relations: qp.data.Dataset, train_filename_to_labels: dict, val_filename_to_labels: dict, class_weights: dict):
    """
    ...
    Parameters:
        trial: 
            Optuna's trial.
        components_cnn_classifier:
            Components CNN.
        relations_cnn_classifier:
            Relations CNN.
        abs_dataset_components (qp.data.Dataset): 
        ...
        class_weights (dict):
            Class weights according to their frequency in the dataset.  
    Returns:
    ...
    ap_ff_layers += [ap_ff_layers[-1] // 2 for _ in range(1, n_ff_layers)]
    ...
    arguments_predictor = ArgumentsPredictorTrainer(
        components_cnn_classifier,
        relations_cnn_classifier,
        ...
        checkpointdir=f'../checkpoints/arguments/optuna/24_11_24-va-f1/trial_{trial.number}/',
    )
    ...
    debug_print(
        ...
        f"  - Components: {c_frozen_layers_percentage}%\n"
    ...
    _, best_results, _ = arguments_predictor.fit(
        abs_dataset_components.training,
        abs_dataset_components.val,
...
def objective_2(trial, components_cnn_classifier, relations_cnn_classifier, abs_dataset_components: qp.data.Dataset, abs_dataset_relations: qp.data.Dataset, train_filename_to_labels: dict, val_filename_to_labels: dict, class_weights: dict):
    """
    Objective function for optuna, used to maximize validation on arguments predictor model. It has refined ranges for choosing some hyperparameters wrt last objective function.
    ...
    # Suggest feedforward layers
    n_ff_layers = trial.suggest_int("n_ff_layers", 2, 3)
    ap_ff_layers = [trial.suggest_categorical("ap_ff_layers0", [256, 128])]
    ap_ff_layers += [ap_ff_layers[-1] // 2 for _ in range(1, n_ff_layers)]

    apdrop_p = trial.suggest_float("apdrop_p", 0.3, 0.5)
    
    # Suggest frozen layer percentages
    c_frozen_layers_percentage = 25
    r_frozen_layers_percentage = 25
    
    # Suggest optimizer
    optimizer_name = trial.suggest_categorical("optimizer", ["Adam", "AdamW", "SGD"])
    weight_decay = trial.suggest_float("weight_decay", 1e-5, 1e-4, log=True)  # Narrowed range
    if optimizer_name == "Adam":
        optimizer_class = optim.Adam
    ...
    arguments_predictor = ArgumentsPredictorTrainer(
        ...
        scheduler_class=scheduler_class,
        scheduler_params=scheduler_params,
        checkpointdir=f'../checkpoints/arguments/optuna/25_11_24-va-f1/trial_{trial.number}/',
    )
...
def objective_3(trial, components_cnn_classifier, relations_cnn_classifier, abs_dataset_components: qp.data.Dataset, abs_dataset_relations: qp.data.Dataset, train_filename_to_labels: dict, val_filename_to_labels: dict, class_weights: dict):
    """
    Objective function for optuna, used to maximize validation on arguments predictor model. It has refined ranges for choosing some hyperparameters wrt last objective function.
    ...
    arguments_predictor = ArgumentsPredictorTrainer(
        ...
        ap_lr=lr,
        criterion=criterion,
        class_weights= class_weights if random_weighted_sampler else None,
        optimizer_class=optimizer_class,
        optimizer_params=optimizer_params,
        scheduler_class=scheduler_class,
        scheduler_params=scheduler_params,
        checkpointdir=f'../checkpoints/arguments/optuna/26_11_24-va-f1/trial_{trial.number}/',
    )
    ...
    debug_print(
        ...
        f"  - Relations: {r_frozen_layers_percentage}%\n"
        f"Scheduler: None\n"
        f"Random sampler: {random_weighted_sampler}\n"
        f"------------------------\n",
        filename=os.path.join(arguments_predictor.checkpointdir, arguments_predictor.checkpointname + '.txt')
    )
...
def objective_test(trial, components_cnn_classifier, relations_cnn_classifier, abs_dataset_components: qp.data.Dataset, abs_dataset_relations: qp.data.Dataset, train_filename_to_labels: dict, val_filename_to_labels: dict, class_weights: dict, glaucoma_collection_components, glaucoma_collection_relations, glaucoma_test_filename_to_labels):
    """
    Objective function for optuna, used to maximize validation on arguments predictor model. It tries to maximize F1 on glaucoma test set instead of validation set.
    ...
    Parameters:
        ...
        class_weights (dict):
            Class weights according to their frequency in the dataset.  
        glaucoma_collection_components (FilenameLabelledColletion):
            Collection of glaucoma set with components (1) and non components (0).
        glaucoma_collection_relations (FilenameLabelledColletion):
            Collection of glaucoma set with relations (1) and non relations (0).
        glaucoma_test_filename_to_labels (dict):
            Dictionary with number of arguments for each file in glaucoma set.
    Returns:
        F1 score on test set for the trial.
    """
    ...
    # Suggest frozen layer percentages
    c_frozen_layers_percentage = trial.suggest_categorical('c_frozen_layers_percentage', [0, 25, 50])
    r_frozen_layers_percentage = trial.suggest_categorical('r_frozen_layers_percentage', [0, 25, 50, 100])
    ...
    random_weighted_sampler = trial.suggest_categorical("sampler", [False, True])
    if random_weighted_sampler:
        criterion=torch.nn.CrossEntropyLoss()
    else:
        criterion=torch.nn.CrossEntropyLoss(weight=class_weights, reduction='mean')
    ...
    arguments_predictor = ArgumentsPredictorTrainer(
        ...
        scheduler_class=scheduler_class,
        scheduler_params=scheduler_params,
        checkpointdir=f'../checkpoints/arguments/optuna/27_11_24-te-f1/trial_{trial.number}/',
    )
    ...
    results_glaucoma = arguments_predictor.evaluate(glaucoma_collection_components, glaucoma_collection_relations, glaucoma_test_filename_to_labels)

    # Return the validation metric for maximization
    return results_glaucoma['F1 Macro']
...
class ArgumentsPredictorTrainer(qp.method.base.BaseQuantifier):
    """
    Trainer for the ArgumentsPredictor model, combining two QuaNet models for component and relationship quantification,
    and training a new head to predict argument count.
    
    Args:
        components_classifier (Classifier): A classifier model for components classification.
        ...
        qc_lr (float, optional): Learning rate for the components classifier. Default is 1e-3.
        ...
        c_frozen_layers_percentage (int, optional): Percentage of the components classifier layers to freeze. Default is 100.
    ...
    def __init__(self, 
                ...
                checkpointdir='../checkpoints/arguments/',
                checkpointname=None,
                criterion = torch.nn.CrossEntropyLoss(),
                class_weights=None,
                ...
                scheduler_params = {}):
        """
        Initializes the ArgumentsPredictorTrainer with the given parameters.
        """
        ...
        if checkpointname is None:
            ...
            self.checkpointname = 'ArgumentsPredictor-'+ timestamp
            self.checkpoint_components_name = 'quanet_components-' + timestamp
        ...
        self.checkpoint_components = os.path.join(checkpointdir, self.checkpoint_components_name+'.pth')
    ...
    def get_filenames_dict(self, filename_to_labels: dict, collections: list[tuple]):
        ...
        filenames_dict = {
            'to_components_instances': defaultdict(list),
            'to_relations_instances': defaultdict(list),
            'to_components_posteriors': defaultdict(list),
    ...
    def fit(self, 
            components_train_data: FilenameLabelledCollection, 
            components_val_data: FilenameLabelledCollection, 
        ...
        """
        Trains the ArgumentsPredictor model on the provided training data.
        
        Args:
            components_train_data (FilenameLabelledCollection): Training data for components classification.
            components_val_data (FilenameLabelledCollection): Validation data for components classification.
        ...
        self._components_classes_ = components_train_data.classes_
        self._relations_classes_ = relations_train_data.classes_

        # estimate the hard and soft stats tpr and fpr of the classifier
        self.components_tr_prev = components_train_data.prevalence()
        self.relations_tr_prev = relations_train_data.prevalence()

        # compute the posterior probabilities of the instances
        components_train_posteriors = self.components_classifier.predict_proba(components_train_data.instances)
        components_valid_posteriors = self.components_classifier.predict_proba(components_val_data.instances)
        components_train_data_embed = FilenameLabelledCollection(self.components_classifier.transform(components_train_data.instances), components_train_data.labels, components_train_data.filenames, self._components_classes_)
        components_valid_data_embed = FilenameLabelledCollection(self.components_classifier.transform(components_val_data.instances), components_val_data.labels, components_val_data.filenames, self._components_classes_)
        ...
        relations_train_data_embed = FilenameLabelledCollection(self.relations_classifier.transform(relations_train_data.instances), relations_train_data.labels, relations_train_data.filenames, self._components_classes_)
        relations_valid_data_embed = FilenameLabelledCollection(self.relations_classifier.transform(relations_val_data.instances), relations_val_data.labels, relations_val_data.filenames, self._components_classes_)

        self.components_quantifiers = {
            'cc': qp.method.aggregative.CC(self.components_classifier).fit(None, fit_classifier=False),
            'acc': qp.method.aggregative.ACC(self.components_classifier).fit(None, fit_classifier=False, val_split=components_val_data),
            'pcc': qp.method.aggregative.PCC(self.components_classifier).fit(None, fit_classifier=False),
            'pacc': qp.method.aggregative.PACC(self.components_classifier).fit(None, fit_classifier=False, val_split=components_val_data),
        }
        ...
        # Istantiate and load weight for components' QuaNet
        nQ = len(self.components_quantifiers)
        nC = components_train_data.n_classes
        self.components_quanet = CustomQuaNetModule(
            doc_embedding_size=components_train_data_embed.instances.shape[1],
            n_classes=components_train_data.n_classes,
            stats_size=nQ*nC,
            order_by=0 if components_train_data.binary else None,
            **self.quanet_params
        ).to(self.device)
        self.components_quanet.load_state_dict(torch.load('../checkpoints/components/Quanet-Components', weights_only=True))
        self.freeze_quanet_layers(self.components_quanet, self.c_frozen_layers_percentage)
        ...
        self.arguments_predictor = ArgumentsPredictor(self.components_quanet, self.relations_quanet,n_classes, dropout_p = self.apdrop_p, ff_layers=self.ap_ff_layers)
        ...
        for label in range(self.arguments_predictor.n_classes):
            ...
            self.optim = self.optimizer_class([
                {'params': filter(lambda p: p.requires_grad, self.components_quanet.parameters()), 'lr': self.qc_lr},
        ...
        collections = [
            (components_train_data_embed, components_train_posteriors, 'to_components_instances', 'to_components_posteriors'),
            (relations_train_data_embed, relations_train_posteriors, 'to_relations_instances', 'to_relations_posteriors'),
            (components_valid_data_embed, components_valid_posteriors, 'to_components_instances', 'to_components_posteriors'),
        ...
        for epoch_i in range(1, self.n_epochs):
            train_matrix = self._epoch(components_train_data_embed, relations_train_data_embed, 
                        self.batch_size, train_filenames, train=True)
            val_matrix = self._epoch(components_valid_data_embed, relations_valid_data_embed, 
            ...
            if early_stop.IMPROVED:
                ...
                torch.save(self.components_quanet.state_dict(), self.checkpoint_components)
            ...
            if early_stop.STOP:
                ...
                self.components_quanet.load_state_dict(torch.load(self.checkpoint_components, weights_only=True))
    ...
    def _retrieve_embeddings(self, filenames: list, filenames_dict: dict, components_data: FilenameLabelledCollection, relations_data: FilenameLabelledCollection):
        """
        ...
        Args:
            ...
            components_data (FilenameLabelledCollection): Collection of components with labels and filenames.
            relations_data (FilenameLabelledCollection): Collection of relations with labels and filenames.

        Returns:
            list: A list of tuples with (components_embedding, relations_embedding, ground_truth).
        """
        embeddings = []

        for filename in filenames:
            # Retrieve instances and posteriors for the current document
            components_sample_instances = filenames_dict['to_components_instances'][filename]
            relations_sample_instances = filenames_dict['to_relations_instances'][filename]
            components_sample_posteriors = filenames_dict['to_components_posteriors'][filename]
            relations_sample_posteriors = filenames_dict['to_relations_posteriors'][filename]

            # Create sample data objects
            components_sample_data = FilenameLabelledCollection(
                components_sample_instances,
                components_data.labels[:len(components_sample_instances)],
                [filename] * len(components_sample_instances)
            )
            ...
            components_quant_estims = self._get_aggregative_estims(components_sample_posteriors, self.components_quantifiers)
            relations_quant_estims = self._get_aggregative_estims(relations_sample_posteriors, self.relations_quantifiers)

            # Create embeddings
            components_embedding = self.components_quanet.quant_embedding(
                components_sample_data.instances, components_sample_posteriors, components_quant_estims
            )
            ...
            embeddings.append((components_embedding, relations_embedding, ground_truth))

        return embeddings

    def _epoch(self, components_data: FilenameLabelledCollection, relations_data: FilenameLabelledCollection, batch_size: int, filenames_dict: dict, train: bool):
        """
        Runs one epoch, computes loss and evaluation metrics.

        Args:
            components_data (FilenameLabelledCollection): Collection of components data for the epoch.
        ...
        filenames = list(filenames_dict['to_components_instances'].keys())
        ...
        for start_idx in pbar:
            ...
            batch_embeddings = self._retrieve_embeddings(batch_filenames, filenames_dict, components_data, relations_data)

            components_batch, relations_batch, gt_batch = zip(*batch_embeddings)
            components_batch = torch.stack(components_batch)
            relations_batch = torch.stack(relations_batch)
            gt_batch = torch.cat(gt_batch)

            if train:
                self.optim.zero_grad()
                pred = self.arguments_predictor.forward(components_batch, relations_batch)
    ...
    def evaluate(self, components_eval_data: FilenameLabelledCollection, relations_eval_data: FilenameLabelledCollection, eval_filename_to_labels: dict, seed: int = 42):
        """
        Evaluates the model on the given dataset and computes various metrics.

        Args:
            components_eval_data (FilenameLabelledCollection): Collection of components data for evaluation.
        ...
        # compute the posterior probabilities of the instances
        components_eval_posteriors = self.components_classifier.predict_proba(components_eval_data.instances)
        components_eval_data_embed = FilenameLabelledCollection(self.components_classifier.transform(components_eval_data.instances), components_eval_data.labels, components_eval_data.filenames, self._components_classes_)
        ...
        eval_filenames = self.get_filenames_dict(
            ...
            collections=[
                (components_eval_data_embed, components_eval_posteriors, 'to_components_instances', 'to_components_posteriors'),
        ...
        eval_matrix = self._epoch(
            components_eval_data_embed, relations_eval_data_embed, 
...
class ArgumentsPredictor(nn.Module):
    """
    ...
    """
    def __init__(self, components_quanet, relations_quanet, n_classes, dropout_p=0.5, ff_layers=[128, 64, 32]):
        super(ArgumentsPredictor, self).__init__()

        # Calculate combined embedding size
        components_stats_size = components_quanet.stats_size
        components_output_size = components_quanet.hidden_size * components_quanet.ndirections
        ...
        self.combined_embedding_size = (components_stats_size + components_output_size) + (relations_stats_size + relations_output_size)
    ...
    def forward(self, components_embedding, relations_embedding):
        """
        ...
        combined_embedding = torch.cat((components_embedding, relations_embedding), dim=-1).view(components_embedding.size(0), -1)
