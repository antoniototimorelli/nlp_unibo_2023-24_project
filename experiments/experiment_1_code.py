...
def split_by_boundaries(text: str, components_boundaries: dict, components_types: dict, positives: list):
    """
...
def label_sentences(text: str, components_boundaries: dict, components_types: dict, positives: list):
    """
    ...
    labeled_parts = split_by_boundaries(text, components_boundaries, components_types, positives)
...
def read_brat_dataset(folder: str, positives: list = ['Claim', 'MajorClaim', 'Premise']):
    """
            ...
            if file.endswith('.ann'):
                ...
                if os.path.exists(txt_path):
                    with open(ann_path, 'r', encoding='utf-8') as ann_f, open(txt_path, 'r', encoding='utf-8') as txt_f:
                        ...
                        labeled_sentences = label_sentences(text, components_boundaries, components_types, positives)
...
def objective(trial, abs_dataset: qp.data.Dataset):
    """
    Objective function for optuna, used to maximize validation on components' CNN.
...
class QuaNetTrainerABS(qp.method._neural.QuaNetTrainer):
    ...
    def fit(self, data: FilenameLabelledCollection, fit_classifier=True):
        ...
        train_data_embed = FilenameLabelledCollection(self.classifier.transform(train_data.instances), train_data.labels, train_data.filenames, self._classes_)
