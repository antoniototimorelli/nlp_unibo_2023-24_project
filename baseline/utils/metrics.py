...
if _has_sklearn:
    ...
    def f1_scores(y_pred, y_true, labelfilter=None, ov75 = False):
        ...
        if not ov75:
            ...
            return {
                'eval_f1_micro_ov75': f1_micro,
                'eval_f1_macro_ov75': f1_macro,
                'clf_report_ov75': clf_report
            }
    ...
    def compute_metrics(task_name, y_pred, y_true, ov75=False):
        assert len(y_pred) == len(y_true)
        if task_name == "seqtag":
            return acc_and_f1(y_pred, y_true)
        elif task_name == "relclass":
            return f1_scores(y_pred, y_true, labelfilter=[0, 1], ov75=ov75)
        elif task_name == "outcomeclf":
            return f1_scores(y_pred, y_true)
        elif task_name == "multichoice":
            return f1_scores(y_pred, y_true, labelfilter=[0, 1])
            # return f1_scores(y_pred, y_true, labelfilter=[0, 1], ov75=ov75)
        else:
            raise KeyError(task_name)
