...
def create_tsv_for_relation_classification(input_dir, output_file, no_pattack=True):
    """
    ...
    for f in os.listdir(input_dir):
        if '.ann' in f:
            ...
            with open(input_dir + f_name + '.ann', 'r', encoding='utf-8') as fann:
                ...
                for line in lines:
                    if no_pattack:  # Check if the variable is set to True
                        line = line.replace('Partial-Attack', 'Attack')
                    line = line.split('\t')
                    if 'T' in line[0]:
...
def main():
    ...
    input_dir = '../../../data/custom_datasets/val/'
    output = 'val'
